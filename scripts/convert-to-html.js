...
Arguments:
  input.md    Path to input markdown file (default: narrative-master-EDITED.md)
  output.html Path to output HTML file (default: input filename with .html extension)
...
Examples:
  ...
  node convert-to-html.js my-story.md
  node convert-to-html.js my-story.md output.html
  node convert-to-html.js ../docs/README.md
`);
    process.exit(0);
}

// Determine input file (from args or default)
let inputPath = args[0] || 'narrative-master-EDITED.md';

// If input path is not absolute, resolve it relative to current directory
...
// Determine output file (from args or auto-generate)
let outputPath;
if (args[1]) {
    outputPath = path.isAbsolute(args[1]) ? args[1] : path.resolve(process.cwd(), args[1]);
} else {
    // Auto-generate output filename: input.md -> input.html
...
const defaultTitle = inputFileName
    .split('-')
