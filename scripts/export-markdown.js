...
function formatList(items, prefix = '-') {
	...
	return (
		items.map((item) => `${prefix} ${htmlToMarkdown(item)}`).join('\n') +
		'\n\n'
	);
}
		...
		pathConfig.lessons.forEach((lessonConfig, lessonIndex) => {
			...
			for (let i = 1; i <= 3; i++) {
				...
				if (chapterData.chapterTitle) {
					markdown += `: ${htmlToMarkdown(chapterData.chapterTitle)}`;
				}
				...
				if (chapterData.characterIntros) {
					...
					markdown += formatCharacterIntros(
				...
				if (
					...
					chapterData.lessonInsight.content
				) {
					markdown += `**${chapterData.lessonInsight.title}**\n`;
					markdown +=
						htmlToMarkdown(chapterData.lessonInsight.content) +
						'\n\n';
				}
				...
				if (
					...
					chapterData.journalEntry.content
				) {
