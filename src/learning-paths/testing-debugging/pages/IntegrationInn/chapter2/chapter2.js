	...
	return (
		<div className='chapter'>
			...
			<StorySection
				paragraphs={[
					<>
						<strong>Cypress</strong> led Aria to a special room in
					...
					<>
						<strong>Aria</strong> examined the mirrors closely.
					...
					<>
						<strong>Debuggora</strong> hooted approvingly from her
						...
						{externalServices.map((service) => (
							<div
								...
								<div className='service-status'>
									{activeMocks.includes(service.id)
										? '✓ Mocked'
										: 'Real Service'}
								</div>
							</div>
							...
							{Object.entries(mockServices).map(
								([service, response]) => (
									<div
										...
										<span className='test-status pass'>
											Active
										</span>
									</div>
								)
			...
			<div className='code-section'>
				<div className='code-header'>
					...
					<div className='code-actions'>
						<button
							onClick={() =>
								createMockService('demo', { status: 200 })
							}>
							Create Demo Mock
						</button>
					</div>
  ...
  expect(mockAnalytics.identify).toHaveBeenCalledBefore(
    mockAnalytics.track
				...
				<div className='code-tooltip'>
					<strong>Mock Creation Wisdom:</strong> "Mocks should be
					simple and focused. Don't try to replicate the entire
					service - just the parts your test needs. Remember: mocks
					are lies that help us find the truth about our code."
				</div>
			</div>
			...
			<ChapterSummary
				lessonInsight={{
					...
					content: (
						<>
							<p>
								Mocking is about control and isolation. By
								replacing external dependencies with predictable
								doubles, you can test your code's behavior
								without worrying about network issues, rate
								limits, or service downtime. It's like
								practicing a play with understudies - you know
								exactly what they'll say and when.
							</p>
							<p>
								The art of mocking lies in balance. Mock too
								little, and your tests become flaky and slow.
								Mock too much, and your tests might pass while
								real integration fails. The key is mocking at
								the right boundaries - typically external
								services, not your own code.
							</p>
						</>
					),
				}}
