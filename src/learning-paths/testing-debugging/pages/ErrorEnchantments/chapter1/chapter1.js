...
const ChapterOne = () => {
	...
	return (
		<div className='chapter'>
			<ChapterIntro chapterNumber={1} title={`The Error Sanctuary`} />

			<StorySection
				paragraphs={[
					<>
						After mastering integration testing at the Inn,{' '}
						<strong>Aria</strong> arrived at the Error Enchantments
					...
					<>
						<strong>Guardian Safiya</strong> welcomed her with a
					...
					<>
						<strong>Debuggora</strong> hooted nervously as a
					...
					<div className='error-types-grid'>
						{errorTypes.map((error) => (
							<div
								...
								onClick={() =>
									!enchantmentActive && enchantError(error)
								}>
...
// - Runtime Errors: Logic errors during execution`}
				...
				<div className='code-tooltip'>
					<strong>Safiya's Teaching:</strong> "Each error type
					requires different handling. Syntax errors prevent
					compilation, runtime errors crash components, and async
					errors can go unnoticed. Know thy enemy to defend against
					it."
				</div>
			</div>
			...
			<ChapterSummary
				...
				lessonInsight={{
					...
					content: (
						<>
							<p>
								Understanding error types is the first step to
								handling them gracefully. In React, errors can
								occur at different stages: during rendering, in
								lifecycle methods, in event handlers, or in
								asynchronous code. Each requires different
								defensive strategies.
							</p>
							<p>
								The key insight: Errors are inevitable, but
								crashes are preventable. By understanding where
								and why errors occur, you can build defensive
								barriers that keep your application running even
