...
const ChapterOne = () => {
	const { runTest, testResults, bugsCaught, testCoverage, detectBug } =
		useOutletContext();
	...
	return (
		<div className='chapter'>
			<ChapterIntro chapterNumber={1} title={`The Bug Manifestation`} />
			...
			<StorySection
				paragraphs={[
					...
					<>
						<strong>Aria</strong> descended into the Underground
						Realms, where the Testing Tower stood as the first line
						of defense. At its peak,{' '}
						<strong>Test Master Jasmine</strong> awaited with her
						companion, <strong>Debuggora</strong>, a wise owl with
			...
			<div className='interactive-section'>
				...
				<InstructionBox
					character={`Debuggora guides you through the Bug Hunter Training Ground.`}>
				...
				{selectedBug && (
					<div className='test-incantation'>
						<div className='incantation-type'>
							{selectedBug.type.replace('-', ' ').toUpperCase()}{' '}
							TEST
						</div>
						<p>
							<strong>Bug Type:</strong> {selectedBug.type}
							<br />
							<strong>Severity:</strong> {selectedBug.severity}
							<br />
							<strong>Test Status:</strong> Protection spell
							activated!
						</p>
					</div>
				)}
				...
				<div className='debuggora-vision'>
					...
					<p>
						Toggle Debuggora's special sight to see the code through
						her eyes:
					</p>
					<button
			...
			<div className='code-section'>
				<div className='code-header'>
					<span className='code-title'>
						Ancient Scroll of Testing
					</span>
					<div className='code-actions'>
						<button onClick={() => runTest('Basic Component Test')}>
							Run Test
						</button>
					</div>
				</div>
...
test('validates input types before processing', () => {
  ...
  expect(input.value).toBe('123');
				...
				<div className='code-tooltip'>
					<strong>Master Jasmine explains:</strong> "Each test is a
					protective ward. The more tests you write, the stronger your
					defenses become. Start with the most critical paths - where
					bugs cause the most damage."
				</div>
			</div>
					...
					content: (
						<>
							<p>
								...
								it's about proving it continues to work as you
								change it. Test Master Jasmine teaches that
								every test is a guardian spell, protecting
								against future bugs. The Testing Tower stands
								not to catch bugs after they appear, but to
								prevent them from manifesting at all.
							</p>
							<p>
								Remember: A bug caught in testing costs minutes
								to fix. A bug caught in production costs hours,
								days, or even your users' trust. Choose your
								battles wisely by testing early and often.
							</p>
						</>
					),
				}}
